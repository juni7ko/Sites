...
if ($w == "u")
{
	...
	//	echo $fsql;
        ...
        if ($row[bf_file])
        {
            $file_script .= "add_file(\" <input type='checkbox' name='bf_file_del[$i]' value='1'>  <img src={$g4[path]}/data/file/".$bo_table."/".$row[bf_file]." width=80> <a href='{$file[$i][href]}'>{$file[$i][source]}({$file[$i][size]})</a>  파일 삭제 ";
