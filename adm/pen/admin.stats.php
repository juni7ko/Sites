...
while ($Data = sql_fetch_array($search_DB))   {
	...
	if($r_year && $r_year != "All")	 {
		if($r_month && $r_month != "All")	 {
			...
			$dayTotalAmount[substr($Data[wr_link1],0,4)."년".substr($Data[wr_link1],4,2)."월"] += $Data[wr_10] ;
			$dayNego[substr($Data[wr_link1],0,4)."년".substr($Data[wr_link1],4,2)."월"] += $Data[wr_10] ;
		...
		$dayTotalAmount[substr($Data[wr_link1],0,4)."년"] += $Data[wr_10] ;
		$dayNego[substr($Data[wr_link1],0,4)."년"] += $Data[wr_10] ;
...
// 객실명 출력
$sql_r = "SELECT r_info_id , r_info_name  from $room_info  order by r_info_name, r_info_id";
	...
	<FORM NAME="SearchForm" METHOD="post" ACTION="<?=$PHP_SELF ?>">
		...
		<TR>
			<TD align="center">
				...
				<?php
				for($Y=2009 ;$Y<(date("Y") + 2);$Y++){
					echo "<option value='$Y'".(($Y == $r_year)?"selected":"").">$Y</option>";
				}?>
			</select>
			년
			<select name="r_month" onChange="if(!this.form.r_year.selectedIndex) this.selectedIndex=0;dateSelect('<?=date('Y')?>',this.value,'')">
	...
	if($mode == 'year'){
		...
		while ($pay2 = sql_fetch_array($select_DB))   {
			...
			<tr>
				...
				<td><?=$pay2['ca_name']?>  <?if($pay2['cnt']-1){ echo "  <span style='color:#3D7A99'> (".$pay2['cnt']." 건)</span>"; }?> </td>
				<td><?=$pay2['wr_link1']?></td>
				<td><?=$pay2['wr_datetime']?></td>
				<td><?=$pay2['wr_name']?> ..</td>
				...
				<!-- <td><?=number_format($pay2['wr_10'] - $insedPay - $insPay)?></td> -->
			</tr>
		...
		while ($pay2 = sql_fetch_array($select_DB))   {
			...
			<tr>
				<td><?=$no?></td>
				<td><a href="<?=$g4['path']?>/bbs/board.php?bo_table=bbs34&wr_id=<?=$pay2['wr_id']?>" target="_blank"><?=$pay2['wr_link1']?></a></td>
				<td><?=$pay2['ca_name']?></td>
				<td><?=$pay2['wr_link1']?></td>
				<td><?=$pay2['wr_datetime']?></td>
				<td><?=$pay2['wr_name']?></td>
				<td><?=($insedPay)?"<font style='color:#999999 ; font-size:11px'>" . number_format($pay2[wr_10]) . "</font><br />":''?><?=number_format($pay2[wr_10] - $insedPay)?></td>
				<td><?=number_format($pay2[wr_10])?></td>
				<!-- <td><?=number_format($pay2['wr_10'] - $insedPay - $insPay)?></td> -->
			</tr>
