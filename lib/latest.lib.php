...
function latest2($skin_dir="", $bo_table, $rows=10, $subject_len=40, $options="")
{
	...
	$tmp_write_table = $g4['write_prefix'] . $bo_table; // 게시판 테이블 전체이름
	$sql = " SELECT * from $tmp_write_table where wr_is_comment = 0 AND mainUse = 1 $options order by rand() limit $rows ";
	//explain($sql);
	$result = sql_query($sql);
	for ($i=0; $row = sql_fetch_array($result); $i++)
		$list[$i] = get_list($row, $board, $latest_skin_path, $subject_len);
...
function bigSale($skin_dir="", $bo_table, $rows=10, $subject_len=40, $options="")
{
	global $g4, $chkLatest;

	$chkP = array();
	foreach ($chkLatest as $value) {
		$chkP[] = " pension_id != $value ";
	}
	$chkQuery = join(" and ", $chkP);
	if( $chkQuery ) $chkQuery = " and " . $chkQuery;
	...
	//explain($sql);
  // 1. 할인율 최대값을 구한다.
	$sql = " SELECT * from $tmp_write_table where wr_is_comment = 0 AND mainUse = 1 $chkQuery order by discount desc limit 0, $rows ";

	$result = sql_query($sql);
	for ($i=0; $row = sql_fetch_array($result); $i++) {
		$list[$i] = get_list($row, $board, $latest_skin_path, $subject_len);
		$chkLatest[] = $list[$i][wr_id];
	}
...
function veryCheep($skin_dir="", $bo_table, $rows=10, $subject_len=40, $options="")
{
	global $g4, $chkLatest;
	...
	//explain($sql);
  // 1. 할인율 최대값을 구한다.
	$sql = " SELECT * from $tmp_write_table where wr_is_comment = 0 AND mainUse = 1 $chkQuery order by lowPrice asc limit 0, $rows ";
...
function hotSale($skin_dir="", $bo_table, $rows=10, $subject_len=40, $options="")
{
	...
	//explain($sql);
	// 1. 할인율 최대값을 구한다.
	$sql = " SELECT * from $tmp_write_table where wr_is_comment = 0 AND mainUse = 1 $chkQuery order by resCount desc limit 0, $rows ";
...
function newStay($skin_dir="", $bo_table, $rows=10, $subject_len=40, $options="")
{
	...
	//explain($sql);
	$sql = " SELECT * from $tmp_write_table where wr_is_comment = 0 AND mainUse = 1 $chkQuery order by wr_num limit 0, $rows ";
