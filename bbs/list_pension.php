...
if (!$sst)
{
    ...
    $sst = preg_match("/^(wr_datetime|wr_hit|wr_good|wr_nogood|lowPrice|highPrice|wr_num)$/i", $sst) ? $sst : "";
}
...
if ($sca || $stx)
{
    $sql = " SELECT distinct wr_parent from $write_table where mainUse = 1 and $sql_search $sql_order limit $from_record, $board[bo_page_rows] ";
}
else
{
    $sql = " SELECT * from $write_table where mainUse = 1 and wr_is_comment = 0 $sql_order limit $from_record, $board[bo_page_rows] ";
}
$result = sql_query($sql);
