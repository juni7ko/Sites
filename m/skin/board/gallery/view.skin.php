...
<article>
    ...
    */
    ...
    <footer style="display: block; margin: 10px auto; text-align: center">
        <?php if ($nogood_href) {?>
        <div id="nogood">
            <div style="color:#888; margin:7px 0 5px 0; font-size: .85em;">비추천 : <?php echo number_format($view['wr_nogood'])?></div>
            <div><a href="<?php echo $nogood_href?>" target="hiddenframe"><img src="<?php echo $board_skin_path?>/img/icon_nogood.gif"></a></div>
        </div>
        <?php } ?>

        <?php if ($good_href) {?>
        <div id="good">
            <div style="color:#888; margin:1px 0 5px 0; font-size: .85em;"><span>추천 : <?php echo number_format($view['wr_good'])?></span></div>
            <div><a href="<?php echo $good_href?>" target="hiddenframe"><img src="<?php echo $board_skin_path?>/img/icon_good.gif"></a></div>
        </div>
        <?php } ?>
...
window.onload=function() {
    resizeBoardImage(<?php echo (int)$board['bo_m_image_width']?>);
//    drawFont();
}
</script>
<!-- 게시글 보기 끝 -->
