...
<table border=0 cellpadding=4 align=center width=100%>
    <form name=fcalendar autocomplete=off>
        ...
        <input type=hidden name=cur_date value='<?php echo $cur_date ?>'>
        <input type=hidden id=delimiter name=delimiter value='<?php echo $delimiter ?>'>
        <tr><td align=center height=30>
                <a href='<?php echo $yyyy_before_href ?>'><<</a>&nbsp;
                <a href='<?php echo $mm_before_href ?>'><</a>
                <?php echo $yyyy_select ?>
                <?php echo $mm_select ?>
                <a href='<?php echo $mm_after_href ?>'>></a>&nbsp;
                <a href='<?php echo $yyyy_after_href ?>'>>></a>
            </td>
        </tr>
        <tr>
            <td align=center>
                <table border=0 cellpadding=4 cellspacing=0 width=100%>
                    <tr align=center>
                        <td width=14% style="color:<?php echo $sunday_color ?>"><?php echo $yoil['0']; ?></td>
                        <td width=14% style="color:<?php echo $weekday_color ?>"><?php echo $yoil['1']; ?></td>
                        <td width=14% style="color:<?php echo $weekday_color ?>"><?php echo $yoil['2']; ?></td>
                        <td width=14% style="color:<?php echo $weekday_color ?>"><?php echo $yoil['3']; ?></td>
                        <td width=14% style="color:<?php echo $weekday_color ?>"><?php echo $yoil['4']; ?></td>
                        <td width=14% style="color:<?php echo $weekday_color ?>"><?php echo $yoil['5']; ?></td>
                        <td width=14% style="color:<?php echo $saturday_color ?>"><?php echo $yoil['6']; ?></td>
                    </tr>
                    <?php $cnt = $day = 0;
                    for ($i = 0; $i < 6; $i++) {
                        echo "<tr>";
        ...
        <tr>
            <td align=center height=30>
                <span style='background-color:<?php echo $today_bgcolor ?>;'>
                    <?php echo  "<a href=\"javascript:date_send('{$today['year']}', '{$mon}', '{$mday}', '{$today['wday']}', '{$yoil[$today['wday']]}');\">"; ?>
                    오늘 : <?php echo "{$today['year']}년 {$today['mon']}월 {$today['mday']}일 ({$yoil[$today['wday']]})"; ?></a>
                </span></td>
        </tr>
    </form>
</table>
