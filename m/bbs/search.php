...
if ($stx) {
    ...
    for ($i = 0; $i < count($g4_search['tables']); $i++) {
        ...
        if ($row['cnt']) {
            ...
            $row2 = sql_fetch($sql2);
            $str_board_list .= "\n<li class='sch_bbs'><a href='{$_SERVER['PHP_SELF']}?$search_query&gr_id=$gr_id&onetable={$g4_search['tables'][$i]}'> &middot; {$row2['bo_subject']} ({$row['cnt']})</a></li>\n";
        }
    }
    ...
    for ($idx = $table_index; $idx < count($search_table); $idx++) {
        ...
        for ($i = 0; $row = sql_fetch_array($result); $i++) {
            ...
            if ($read_level[$idx] <= $member['mb_level']) {
                $content = cut_str(get_text($row['wr_content']), 200, "..");
            ...
            $list[$idx][$i]['name'] = get_sideview($row['mb_id'], cut_str($row['wr_name'], $config['cf_m_cut_name']), $row['wr_email'], $row['wr_homepage']);
    ...
    $write_pages = get_paging($config['cf_m_write_pages'], $page, $total_page, "{$_SERVER['PHP_SELF']}?$search_query&gr_id=$gr_id&srows=$srows&onetable=$onetable&page=");
...
$search_skin_path = "{$g4['g4m_path']}/skin/search/{$config['cf_m_search_skin']}";
