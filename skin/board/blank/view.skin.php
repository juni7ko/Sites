...
<style type="text/css">
.color_text { color:<?=$brd_text_color?>; }
...
<div style="clear:both; height:30px;">
    ...
    <span class="color_text">작성일 : <?=date("y-m-d H:i", strtotime($view[wr_datetime]))?></span>
    </div>
...
<div style="border:1px solid <?=$brd_line_color?>; clear:both; height:34px;">
    <table border=0 cellpadding=0 cellspacing=0 width=100%>
    <tr>
        <td style="padding:8px 0 0 10px;">
            <div class="color_text" style="font-size:13px; font-weight:bold; word-break:break-all;">
...
<table border=0 cellpadding=0 cellspacing=0 width=100%>
<tr>
    <td class="color_text bottom_white" height=30>
...
for ($i=0; $i<count($view[file]); $i++) {
    if ($view[file][$i][source] && !$view[file][$i][view]) {
        $cnt++;
        echo "<tr><td height=30 class='color_text bottom_white'>";
        echo "&nbsp;&nbsp;<img src='{$board_skin_path}/img/icon_file.gif' align=absmiddle border='0'>";
        echo "<a href=\"javascript:file_download('{$view[file][$i][href]}', '{$view[file][$i][source]}');\" title='{$view[file][$i][content]}'>";
        echo "&nbsp;<span class='color_text'>{$view[file][$i][source]} ({$view[file][$i][size]})</span>";
        echo "&nbsp;<span class='color_text' style=\"font-size:11px;\">[{$view[file][$i][download]}]</span>";
        echo "&nbsp;<span class='color_text' style=\"font-size:11px;\">DATE : {$view[file][$i][datetime]}</span>";
        echo "</a></td></tr>";
    }
}
...
for ($i=1; $i<=$g4[link_count]; $i++) {
    if ($view[link][$i]) {
        ...
        echo "<tr><td class='color_text bottom_white' height=30>";
        echo "&nbsp;&nbsp;<img src='{$board_skin_path}/img/icon_link.gif' align=absmiddle border='0'>";
        echo "<a href='{$view[link_href][$i]}' target=_blank>";
        echo "&nbsp;<span class='color_text'>{$link}</span>";
        echo "&nbsp;<span class='color_text' style=\"font-size:11px;\">[{$view[link_hit][$i]}]</span>";
        echo "</a></td></tr>";
    }
}
?>
<tr> 
    <td class='color_text bottom_white' height="150" style="word-break:break-all; padding:10px;">
        <?php
// 파일 출력
        for ($i=0; $i<=count($view[file]); $i++) {
            if ($view[file][$i][view]) 
...
//echo $view[rich_content]; // {이미지:0} 과 같은 코드를 사용할 경우?>
        ...
        <div class='color_text' style="margin:7px 0 5px 0;">비추천 : <?=number_format($view[wr_nogood])?></div>
        ...
        <?php if ($good_href) {?>
        <div style="width:72px; height:55px; background:url(<?=$board_skin_path?>/img/good_bg.gif) no-repeat; text-align:center; float:right;">
        <div class='color_text' style="margin:7px 0 5px 0;"><span style='color:crimson;'>추천 : <?=number_format($view[wr_good])?></span></div>
...
<?php if ($is_signature) { echo "<tr><td class='color_text' align='center' style='border-bottom:1px solid {$brd_line_color}; padding:5px 0;'>$signature</td></tr>"; } // 서명 출력 ?>
</table>
<br>
