...
$wr_reserv_print = $wr_reserv_print . Get_Option_list($bo_table, "chk_list",$_POST[chk_wr_op],$_POST[chk_wr_op_cnt]);

$total_p = $total_p + Get_Option_list($bo_table, "total_cost",$_POST[chk_wr_op],$_POST[chk_wr_op_cnt]);
...
?>
  ...
  <?php if ($is_guest) { ?>
  <tr>
    <td class="consjbox"><img id='kcaptcha_image' /></td>
    <td colspan="3" align="left"><input class='ed' type=input size=10 name=wr_key itemname="자동등록방지" required>&nbsp;&nbsp;왼쪽의 글자를 입력하세요.</td>
  </tr>
	<?php } ?>
</table>
<?php if ($is_secret) { ?>
