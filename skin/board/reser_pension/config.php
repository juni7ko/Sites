...
*/
function Get_Option_list($bo_table, $type, $op_id, $op_cnt) {
	global $bo_table, $write_table;
	...
	if($type == "chk_list" || $type == "total_cost") {
		...
		for ($i=count($tmp_array)-1; $i>=0; $i--) 
		{
				$option_sql = " SELECT * FROM {$write_table}_r_option WHERE r_op_id = '{$tmp_array[$i]} LIMIT 1' ;";
				$r_op = sql_fetch($option_sql);
				$op_print = $op_print . "<dt style='padding-left:10px;'>- " . $r_op[r_op_name] . " : " . number_format($r_op[r_op_cost]) . "원";
				
				if($tmp_cnt[$tmp_array[$i]]) {
					$tmp_cost[$i] =  $r_op[r_op_cost] * $tmp_cnt[$tmp_array[$i]];
					$op_print = $op_print . " X " . $tmp_cnt[$tmp_array[$i]] . " = " . number_format($tmp_cost[$i]) . "원";
					$op_cost = $op_cost + $tmp_cost[$i];
				} else {
					$op_cost = $op_cost + $r_op[r_op_cost];
				}
				
				$op_print = $op_print . "</dt>";
		}
		...
		for ($i=0; $r_op = sql_fetch_array($result_op); $i++)  {
			echo "<dt>";
			echo "<label><input type=checkbox name='chk_wr_op[]' value='{$r_op[r_op_id]}'>";

			echo "<select name='chk_wr_op_cnt[{$r_op[r_op_id]}]' size='1' value=''>";
			for($k=1; $k <= 10; $k++) {
				echo "<option value='{$k}'>{$k}</option>";
			}
			echo "</select>";

			echo " {$r_op[r_op_name]} : ".number_format($r_op[r_op_cost])."원</label>";
			echo "</dt>";
		}
		return;
	}
