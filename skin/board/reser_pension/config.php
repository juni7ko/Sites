...
function Get_Date_Close($time,$r_name) {
	...
	$sql = " SELECT r_close_name FROM {$write_table}_r_close WHERE  pension_id = '$pension_id' and ($time BETWEEN r_close_date AND r_close_date2)";
...
function Get_Date_Cost($time, $r_name) {
	...
	if($type) {
		...
		$r_id = sql_fetch(" SELECT r_info_id FROM {$write_table}_r_info WHERE pension_id = '$pension_id' and  r_info_name = '$r_name' LIMIT 1");
		$Cost = sql_fetch(" SELECT $field FROM {$write_table}_r_cost WHERE pension_id = '$pension_id' and  r_info_id = '$r_id[r_info_id]' LIMIT 1");
	}
...
function Get_Date_Reserv($bo_table, $r_name, $time, $link_url) {
	...
	$chk_sql = " SELECT sum(wr_9) as cnt FROM {$write_table} WHERE  pension_id = '$pension_id' and ca_name='$r_name' and wr_link1 <= '$time' and wr_link2 > '$time'";
	...
	if(Get_Date_Close($pdate,$r_name) == $r_name) {
		...
		if(Get_Room_Info_One($bo_table, $r_name, 'over') == "O") {
			...
			if($row[cnt]) {
				$sql2 = " SELECT wr_4 FROM {$write_table} WHERE  pension_id = '$pension_id' and ca_name='$r_name' and wr_4!='예약취소' and wr_link1 <= '$time' and wr_link2 > '$time'";
				$row2 = sql_fetch($sql2);
				if($row2['wr_4'] == "예약확인중") {
					$r_print = "{$rstate_dae}{$r_name_dae}";
