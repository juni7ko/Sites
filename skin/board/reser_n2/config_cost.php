...
if($u == "edit") {
	if($cost_info) {
		$sql = "UPDATE {$write_table}_r_cost SET r_cost_11 = '$r_cost_11',
			...
			r_cost_13 = '$r_cost_13',
			r_cost_14 = '$r_cost_14',
			r_cost_15 = '$r_cost_15' WHERE r_info_id ='$id' LIMIT 1 ;";
	} else {
		$sql = "INSERT INTO {$write_table}_r_cost (r_info_id, r_cost_11, r_cost_12, r_cost_13, r_cost_14, r_cost_15) 
			VALUES ('$id', '$r_cost_11', '$r_cost_12', '$r_cost_13', '$r_cost_14', '$r_cost_15');";
	}
	
	$result = sql_fetch($sql);
	alert("수정완료!!","{$board_skin_path}/config_room.php?bo_table={$bo_table}");
...
function Process(u,id) {
	f = document.go_cost;
	if(u == "cost") {
		f.action = "./config_cost.php?bo_table=<?=$bo_table?>";
	} else {
		return false;
	}
