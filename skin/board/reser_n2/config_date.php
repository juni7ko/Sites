...
<div style='margin-top:5px; text-align:right;'><input type=button class='<?=$css[btn]?>' value='추가' onClick="Process('add',0); return false;"></div>
<?php
## 리스트 끝
}
?>
...
<?php
// 방 갯수마다 모두 요금을 입력받는다.
$sql_room = " SELECT * FROM {$write_table}_r_info order by r_info_order ASC ";
...
<?php } else if($u == "edit") {
	## 업데이트 리스트
	$r_date = sql_fetch(" SELECT * FROM {$write_table}_r_date WHERE r_date_idx='$id' ");
?>
...
for ($ri=0; $r_info = sql_fetch_array($result_room); $ri++)  {
	$rlist = $ri%2;
	$date_cost = sql_fetch("SELECT * FROM {$write_table}_r_date_cost WHERE r_date_idx = '$r_date[r_date_idx]' and r_info_id = '$r_info[r_info_id]'");
?>
<tr class="ht center list<?=$rlist?>">
...
## 업데이트 리스트 끝
	...
	if($u == "del") {
		$sql = "DELETE FROM {$write_table}_r_date WHERE r_date_idx ='$id'";
		sql_query($sql);	
	} else if($u == "update") {
		...
		for($i=0; $i < count($r_dcost_idx); $i++ ) {
			if($r_dcost_idx[$i]) {
				$sql = "UPDATE {$write_table}_r_date_cost SET
					...
					WHERE r_dcost_idx = '$r_dcost_idx[$i]'";
				$result = sql_query($sql);
			} else {
				$sql = " SELECT r_date_idx FROM {$write_table}_r_date WHERE r_date_name = '$r_date_name' AND r_date_sdate = '$sdate' AND r_date_edate = '$edate' ";
				$r_date_idx = sql_fetch($sql);
				
				$sql = "INSERT INTO {$write_table}_r_date_cost 
					(r_dcost_idx, r_info_id, r_date_idx, r_date_cost_1, r_date_cost_2, r_date_cost_3, r_date_cost_4, r_date_cost_5) VALUES 
					('','$r_info_id[$i]', '$r_date_idx[r_date_idx]', '$r_date_cost_1[$i]', '$r_date_cost_2[$i]', '$r_date_cost_3[$i]', '$r_date_cost_4[$i]', '$r_date_cost_5[$i]');";
				sql_query($sql);
			}
		}
	} else if($u == "insert") {
		...
		$sql = "INSERT INTO {$write_table}_r_date (r_date_name, r_date_sdate, r_date_edate) VALUES ('$r_date_name', '$sdate', '$edate');";
		sql_query($sql);
