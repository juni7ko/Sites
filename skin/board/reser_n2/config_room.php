...
function Process(u,id) {
	...
	if(u == "cost") {
		f.action = "<?=$board_skin_path?>/config_cost.php?bo_table=<?=$bo_table?>";
	} else {
		f.action = "<?=$this_page?>";
	}
...
<?php if($u == "add") {
	...
	if($u == "del") {
		...
		$sql = "UPDATE {$write_table}_r_info SET r_info_name = '$r_info_name',
			...
			r_info_multi = '$r_info_multi' WHERE r_info_id ='$id' LIMIT 1 ;";
	
		$result = sql_query($sql);
		Up_Cate($bo_table);
	} else if($u == "insert") {
		$r_info_person3 = $r_info_person2 - $r_info_person1;
		$sql = "INSERT INTO {$write_table}_r_info (r_info_name, r_info_area, r_info_person1, r_info_person2, r_info_person3, r_info_person_add, r_info_cnt, r_info_over, r_info_order, r_info_multi) VALUES ('$r_info_name', '$r_info_area', '$r_info_person1', '$r_info_person2', '$r_info_person3', '$r_info_person_add', '$r_info_cnt', '$r_info_over', '$r_info_order', '$r_info_multi');";
		sql_query($sql);
		Up_Cate($bo_table);
	}
