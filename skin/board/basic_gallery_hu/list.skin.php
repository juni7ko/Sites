...
$thumb_w = $board[bo_1]; //썸네일넓이
$thumb_h = $board[bo_2];   //썸네일높이

// li 크기를 썸네일 크기에 맞춰서 변경
$li_w = $thumb_w + 10;
$li_h = $thumb_h + 120;
...
<ul class="hd-board-list-gallery">
	...
	?>
	<li<?php echo $tr_class?> style="width:<?php echo $li_w;?>px; height:<?php echo $li_h;?>px;">
		<div class="thumb" style="width:<?php echo $thumb_w;?>px; height:<?php echo $thumb_h;?>px;">
			<a href="<?php echo $list[$i]['href'];?>"><img src="<?php echo $thumb[0];?>" alt="<?php echo $list[$i]['subject'];?>" /></a>
			<?php if($original[0]){?>
			<a href="<?php echo $original[0];?>" rel="gallery_zoom" class="zoom" title="<?php echo $list[$i]['subject'];?>">확대보기</a>
			<?php }?>
		</div>
		<div class="subject" style="padding-top:<?php echo $thumb_h+25;?>px;">
			<?php if ($is_checkbox) { ?><input type="checkbox" name="chk_wr_id[]" class="iCheckbox" value="<?php echo $list[$i]['wr_id']?>" /><?php } ?>
...
//			echo $list[$i]['icon_secret'] ? " ".$list[$i]['icon_secret'] : "";
		...
		if ($is_category && $list[$i]['ca_name']) {
			echo "<div class=\"category\"><a href='{$list[$i]['ca_name_href']}'>{$list[$i][ca_name]}</a></div>";
		}
		?>
		<div class="author">
		...
		<div class="info">
			<?php echo $date?> / hit <?php echo $list[$i]['wr_hit']?>
		</div>
		<div class="good">
			<?php if ($is_good) { ?>추천 <span class="good"><?php echo $list[$i]['wr_good']?></span><?php }?>
			<?php if ($is_nogood) { ?>비추천 <span class="nogood"><?php echo $list[$i]['wr_nogood']?></span><?php }?>
		</div>
	</li>
    <?php } // for()?>
    <?php if (count($list) == 0) { ?>
