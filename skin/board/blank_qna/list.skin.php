...
.board_list th { border-top:2px solid <?=$brd_line_color;?>; border-bottom:2px solid <?=$brd_line_color;?>; }
...
.board_list td { border-bottom:1px solid <?=$brd_line_color;?>; } 
...
.board_search .stx { height:21px; border:1px solid <?=$brd_line_color;?>; border-right:1px solid <?=$brd_line_color;?>; border-bottom:1px solid <?=$brd_line_color;?>; }
...
<table width="<?=$width?>" align="center" cellpadding="0" cellspacing="0"><tr><td>
    ...
    <col width="50" />
    <?php if ($is_checkbox) { ?><col width="40" /><?php } ?>
    <col />
    <col width="110" />
    <col width="40" />
    <col width="70" />
    ...
    <tr>
        <th>번호</th>
        <?php if ($is_checkbox) { ?><th><input onclick="if (this.checked) all_checked(true); else all_checked(false);" type="checkbox"></th><?php }?>
        <th>제&nbsp;&nbsp;&nbsp;목</th>
        <th>글쓴이</th>
        <th><?=subject_sort_link('wr_datetime', $qstr2, 1)?>날짜</a></th>
        <th><?=subject_sort_link('wr_link2', $qstr2, 1)?>처리상태</a></th>
        <?php if ($is_good) { ?><th><?=subject_sort_link('wr_good', $qstr2, 1)?>추천</a></th><?php }?>
        <?php if ($is_nogood) { ?><th><?=subject_sort_link('wr_nogood', $qstr2, 1)?>비추천</a></th><?php }?>
    </tr>
    ...
    <tr class="bg<?=$bg?>"> 
        ...
        <?php if ($is_checkbox) { ?><td class="checkbox"><input type=checkbox name=chk_wr_id[] value="<?=$list[$i][wr_id]?>"></td><?php } ?>
        <td class="subject">
            <?php echo $nobr_begin;
            echo $list[$i][reply];
        ...
        <td class="hit"><?=($list[$i][wr_link2]) ? $list[$i][wr_link2]:"접수"?></td>
        <?php if ($is_good) { ?><td class="good"><?=$list[$i][wr_good]?></td><?php } ?>
        <?php if ($is_nogood) { ?><td class="nogood"><?=$list[$i][wr_nogood]?></td><?php } ?>
    </tr>
    <?php } // end for ?>

    <?php if (count($list) == 0) { echo "<tr><td class='color_text' colspan='$colspan' height=100 align=center>게시물이 없습니다.</td></tr>"; } ?>
