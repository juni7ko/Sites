...
$nDateTmp = mktime(12,0,0,$nDateM,$nDateD,$nDateY);
	...
	$prevLink = "{$url}?pension_id=$pension_id&bo_table=$bo_table&wr_id=$wr_id&sDate=$prevDay";
	$nextLink = "{$url}?pension_id=$pension_id&bo_table=$bo_table&wr_id=$wr_id&sDate=$nextDay";
...
?>
			...
			<tr>
				<th colspan="15">
					<input type="button" value="Prev" onclick="pageLoader('view.skin.ajax.php',$prevDay);">
					<input type="button" value="이전" onClick="location.href='<?=$prevLink?>'" />
					<a href="<?="{$url}"?>" title="오늘로" onfocus="this.blur()">
