...
//	echo  $list[$i]['pension_id'];
				...
				<td>
					<?=$r_info['r_info_area']?>평(<?=$r_info['r_info_area'] * 3.3?>㎡)<br />
					원룸형(온돌룸1개+화장실1개)<br />
					<font color=#777777><?=cut_str(strip_tags($list[$i]['wr_content']),210,"…")?></font>
				</td>
				<td>
					기준<?=$r_info['r_info_person1']?>명<br />
					최대<?=$r_info['r_info_person2']?>명
				</td>
				<td>
					<?=$r_info['r_info_person2']?>명초과시<br />
					인원당<?=$r_info['r_info_person_add']?>원
				</td>
				<td class="last"> <!-- 요금 -->
					<?=$r_cost['r_cost_11']?>
				</td>
