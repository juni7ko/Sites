...
function show_list() {
	...
	$colspan = 5;
## 리스트 시작
	$sql = " SELECT * FROM {$write_table}_r_tel order by r_tel_date ASC ";
...
<?php if($u == "add") {
	...
	echo "<td>".Get_Room_Select($pen_id,'r_tel_name','')."</td>";
	...
	echo "<tr align=center height=30>";
	echo "<td>".Get_Room_Select($pen_id,'r_tel_name',$r_date[r_tel_name])."</td>";
	...
	if($u == "del") {
		...
		$sql = "UPDATE {$write_table}_r_tel SET r_tel_name = '$r_tel_name',
			r_tel_date = '$sdate', r_tel_date2 = '$sdate2' WHERE r_tel_idx ='$id' LIMIT 1 ;";
		...
		$sql = "INSERT INTO {$write_table}_r_tel (r_tel_name, r_tel_date, r_tel_date2) VALUES ('$r_tel_name', '$sdate', '$sdate2');";
		sql_query($sql);
	}
