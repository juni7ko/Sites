...
<!--     <?php if ($is_category) { ?><form name="fcategory" method="get"><div><select name=sca onchange="location='<?=$category_location?>'+this.value;"><option value=''>전체</option><?=$category_option?></select></div></form><?php } ?>
 -->
...
$list[$i][subject] = str_replace(" ","&nbsp;",$list[$i][subject]);
   ...
   if (file_exists($thumb) && $list[$i][file][0][file]) {
			 $img = "<a href='$g4[path]/data/file/$bo_table/$image'  title=".$list[$i][subject]."><img src='{$thumb}' width='{$thumb_width}'></a>";
		} else {
