                                ...
                                <td><table width="100%" border="0" align="center" cellpadding="5" cellspacing="0">
                                        <tr>
                                            ...
                                            <td align="center"><table width="200" border="0" cellpadding="3" cellspacing="0">
                                                    <tr>
                                                        <td align="center"><a href="<?="$_SERVER[PHP_SELF]?bo_table=$bo_table&view_mode=call2"?><?php if ($month == 1) { $year_pre=$year-1; $month_pre=12; } else {$year_pre=$year; $month_pre=$month-1;} echo ("&year=$year_pre&month=$month_pre");?>" target="_self" onfocus="this.blur()"><img src="<?=$board_skin_path?>/img_n/pre.gif" width="30" height="14" /></a></td>
                                                        <td align="center"><a href="<?="$_SERVER[PHP_SELF]?bo_table=$bo_table&view_mode=call2"?>" title="오늘로" onfocus="this.blur()"><span style="font-size:16px; font-weight:bold;">
                                                            <?=$year?>
                                                            년
                                                            <?=$month?>
                                                            월</span></a></td>
                                                        <td align="center"><a href="<?="$_SERVER[PHP_SELF]?bo_table=$bo_table&view_mode=call2"?><?php if ($month == 12) { $year_pre=$year+1; $month_pre=1; } else {$year_pre=$year; $month_pre=$month+1;} echo ("&year=$year_pre&month=$month_pre");?>" target="_self" onfocus="this.blur()"><img src="<?=$board_skin_path?>/img_n/next.gif" width="30" height="14" /></a></td>
                                                    </tr>
                                                </table></td>
...
##################################### 기존내용
	...
	for ($iz = 1; $iz <= $lastcount; $iz++) {
		...
		if ($dayoftheweek < $iz  &&  $iz <= $lastday[$month]+$dayoftheweek)	{ // 전체 루프안에서 숫자가 들어가는 셀들만 해당됨. 즉 11월 달에서 1일부터 30 일까지
			...
			//$daytext = "<a href='#' onclick='viewCallPop(\"{$board_skin_path}/list.call.popup2.php?bo_table={$bo_table}&pdate={$pdate}\",800,500);'>$daytext</a>";
			$daytext = "<a href='#' onclick='viewCallPop(\"{$board_skin_path}/list.call.popup2.php?bo_table={$bo_table}&pdate={$f_date}\",800,500);'>$daytext</a>";
...
## Juni7 예약 멀티 체크 & 방 리스트 출력
			...
			for($c=0; $c<count($r_name); $c++) {//기본 카테고리 출력
				echo Get_Date_Reserv_List($bo_table, $r_name[$c], $rc_date, "");
			}
			echo ("</td>\n");  // 한칸을 마무리
			$cday++; // 날짜를 카운팅
