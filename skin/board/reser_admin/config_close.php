...
function viewCloseRoom($penID, $pDate)
{
	global $write_table2;
	// 방 정보 수집
...
if($u) {
	...
	if($cidx) {
		$sql = " DELETE FROM g4_write_bbs34_r_close WHERE pension_id = '$pension_id' AND r_close_idx ='$cidx' and r_info_id = '$ridx' and ($pDate BETWEEN r_close_date AND r_close_date2) ";
		sql_query($sql);
	} else {
		$sql = "INSERT INTO g4_write_bbs34_r_close (r_close_name, r_close_date, r_close_date2, pension_id, r_info_id) VALUES ('$info[r_info_name]', '$pDate', '$pDate', '$pension_id', '$ridx' );";
		sql_query($sql);
	}
}
?>
...
?>
	...
	<div>
		<span class="resClose1">가</span>예약가능
		<span class="resClose2">완</span>관리자예약
	</div>
	<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		...
		for ($iz = 1; $iz <= $lastcount; $iz++) {
			...
			if ($dayoftheweek < $iz  &&  $iz <= $lastDay[$sDateM]+$dayoftheweek) {
				...
				$rList = viewCloseRoom($pension_id, $tDateTmp);
				for($rcnt = 0; $rcnt < count($rList); $rcnt++) {
					$resClass = ( $rList[$rcnt]['r_close_idx'] ) ? "resClose2" : "resClose1";
					?>
						<li class="resCloseLi" onClick="changeState('<?=$tDateTmp?>','<?=$rList[$rcnt][r_info_id]?>','<?=$rList[$rcnt][r_close_idx]?>')">
							<span class="<?=$resClass?>"><?=($rList[$rcnt]['r_close_idx'])?"완":"가";?></span><?=$rList[$rcnt]['r_info_name']?>
						</li>
					<?php
				}
				echo "</div></ul>";
