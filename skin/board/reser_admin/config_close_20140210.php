...
<script type="text/javascript">
	$(function() {
		...
		$("#r_close_date").datepicker();
		$("#r_close_date2").datepicker();
	});
</script>
...
function show_list() {
	...
	$colspan = 5;
## 리스트 시작
	$sql = " SELECT * FROM {$write_table}_r_close where pension_id = '$pension_id'  order by r_close_date ASC ";
	...
	for ($i=0; $r_date = sql_fetch_array($result); $i++)  {
		$j = $i+1;
		echo "<tr class='ht center list$i'>";
		echo "<td>" . $j . "</td>";
...
<?php if($u == "add") {
	$colspan = 3;
	echo "<input type='hidden' name='r_info_name' id='r_info_name' value='' />";
	...
	echo "<td><input class=m_text id=r_close_date type=text required numeric itemname=시작일 size=13 name=r_close_date value='" . date("Ymd", mktime()) . "' readonly title='옆의 달력 아이콘을 클릭하여 날짜를 입력하세요.'> <a href=\"javascript:win_calendar('r_close_date', document.getElementById('r_close_date').value, '');\"><img src='./img/calendar.gif' border=0 align=absmiddle title='달력 - 날짜를 선택하세요'></a></td>";
	echo "<td><input class=m_text id=r_close_date2 type=text required numeric itemname=종료일 size=13 name=r_close_date2 value='" . date("Ymd", mktime()) . "' readonly title='옆의 달력 아이콘을 클릭하여 날짜를 입력하세요.'> <a href=\"javascript:win_calendar('r_close_date2', document.getElementById('r_close_date2').value, '');\"><img src='./img/calendar.gif' border=0 align=absmiddle title='달력 - 날짜를 선택하세요'></a></td>";
	...
	if($u == "del") {
		...
		$sql = "UPDATE {$write_table}_r_close SET r_close_date = '$sdate', r_close_date2 = '$sdate2' WHERE r_close_idx ='$id' LIMIT 1 ;";
		...
		$sql = "INSERT INTO {$write_table}_r_close (r_close_name, r_close_date, r_close_date2, pension_id, r_info_id) VALUES ('$info[r_info_name]', '$sdate', '$sdate2', '$pension_id', '$r_info_id' );";
		sql_query($sql);
	}
