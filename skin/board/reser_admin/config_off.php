...
<div>공휴일로 지정한 전날 요금이 공휴일 요금으로 계산됩니다.</div>
<?php
function show_list() {
	global $g4, $write_table, $css, $pension_id;
## 리스트 시작
	$sql = " SELECT * FROM {$write_table}_r_off where pension_id = '$pension_id' order by r_off_date ASC ";
...
<?php if($u == "add") {
	...
	$colspan = 4;
	## 업데이트 리스트
	$r_date = sql_fetch(" SELECT * FROM {$write_table}_r_off where pension_id = '$pension_id' and r_off_idx='$id' ");
	echo "<table width='100%' border='0' cellpadding='3' cellspacing='1' class='$css[table]'>";
	echo "<tr class='$css[tr]'>
				<td>공휴일명</td>
				<td>시작일</td>
	...
	if($u == "del") {
		...
		$sql = "INSERT INTO {$write_table}_r_off (r_off_name, r_off_date, r_off_date2, pension_id) VALUES ('$r_off_name', '$sdate', '$sdate2', '$pension_id');";
		sql_query($sql);
	}
