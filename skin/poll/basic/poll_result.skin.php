...
<?php if ($is_etc) { ?>
    ...
    <?php for ($i=0; $i<count($list2); $i++) { ?>
        ...
        <tr> 
            ...
            <td width="70" align="center" bgcolor="#FAFAFA"><?php if ($list2[$i][del]) { echo $list2[$i][del] . "<img src='$g4[bbs_img_path]/btn_comment_delete.gif' width=45 height=14 border=0></a>"; } ?></td>
...
<?php } ?>
        ...
        <tr> 
            ...
            <td width="560"><select name=po_id onchange="select_po_id(this)"><?php for ($i=0; $i<count($list3); $i++) { ?><option value='<?=$list3[$i][po_id]?>'>[<?=$list3[$i][date]?>] <?=$list3[$i][subject]?><?php } ?></select><script>document.fpolletc.po_id.value='<?=$po_id?>';</script></td>
        </tr>
        </form>
        <tr> 
